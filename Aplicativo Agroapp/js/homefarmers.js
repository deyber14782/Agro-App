...
.then((snapshot)=>{
    if(snapshot.exists()){
        for(var i=0;i<snapshot.val().FarmerProduct.length;i++){
            ...
            //Asignar valores
            img.src=snapshot.val().FarmerProduct[i].Url
            h2.textContent=snapshot.val().FarmerProduct[i].Titulo
            span.textContent=snapshot.val().FarmerProduct[i].Informacion+' - $'+snapshot.val().FarmerProduct[i].Precio
            li.textContent=snapshot.val().FarmerProduct[i].Ubicacion
            ...
            button.addEventListener('click',(e)=>{
                ...
                .then((snapshot)=>{
                    if(snapshot.exists()){
                        ...
                        for(var i=0;i<snapshot.val().FarmerProduct.length;i++){
                            if(snapshot.val().FarmerProduct[i].Url==urlImage){
                                idFarmer=snapshot.val().FarmerProduct[i].Id
                                
                                get(child(db,'User/Farmer/'+idFarmer))
                                .then((snapshot)=>{
                                    if(snapshot.exists()){
                                        Swal.fire({
